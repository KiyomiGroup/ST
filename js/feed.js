  ...
  try {
    ...
    if (posts && posts.length > 0) {
      ...
      feedState = posts.map((p, i) => ({
        id:          String(p.id),
        author:      p.resolved_author_name || 'Tasker',
        initials:    (p.resolved_author_name || 'TK').replace(/[^a-zA-Z ]/g,'').trim().split(' ').filter(Boolean).map(w=>w[0]).join('').slice(0,2).toUpperCase() || 'T',
        avatarClass: `av-${(i % 6) + 1}`,
        service:     p.service || '',
        location:    p.location || 'Lagos',
        timeAgo:     timeAgo(p.created_at),
        caption:     p.resolved_caption || '',
        mediaColor:  p.resolved_image ? null : colors[i % colors.length],
        imageUrl:    p.resolved_image || null,
        mediaIcon:   'img',
