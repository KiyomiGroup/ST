  ...
  try {
    ...
    if (posts && posts.length > 0) {
      ...
      feedState = posts.map((p, i) => ({
        ...
        initials:    (p.author_name || 'TK').replace(/[^a-zA-Z ]/g,'').trim().split(' ').filter(Boolean).map(w=>w[0]).join('').slice(0,2).toUpperCase() || 'T',
        avatarClass: `av-${(i % 6) + 1}`,
        service:     p.service || '',
        location:    p.location || 'Lagos',
        timeAgo:     timeAgo(p.created_at),
        caption:     p.content || p.caption || '',
        mediaColor:  (p.image || p.image_url) ? null : colors[i % colors.length],
